...
https://github.com/MarshallBuchner/kept/pull/25–#30 + [#38](https://github.com/MarshallBuchner/kept/pull/38) are **merged** (green **Welcome** landing, paper **Home**, full-bleed sage home-screen icon, Archive folders, CompleteRegistration + `ttclid`, Get Started → `/?welcome=1`, creatives/runbook, Pixel create path, Checkout header forced to **Kept**). Production already serves `/welcome`.
